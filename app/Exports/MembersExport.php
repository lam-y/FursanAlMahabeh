    ...
    public function collection()
    {
        ...
        return $members->map(function ($member) {
            return [
                ...
                'أوسمة (هوايات)' => $member->hobbyBadges->pluck('name')->implode(', '),
                'أوسمة فرع' => $member->branchBadges->pluck('name')->implode(', '),
                'نوع العضوية' => $member->memberType->name ,
                'ترفيع' => $member->promoted_text,
                'الفرع' => optional($member->branch)->name,
                'توتيم' => $member->totem_text,
                'التوتيم' => $member->totem_name,
                'أوسمة عامة' => $member->publicBadges->pluck('name')->implode(', '),
            ];
        });
    }
