        ...
        <div class="well well-sm existing-file">
            @foreach ($values as $key => $value)
                <div class="file-preview">
                    @php
                        $filePath = 'posts/' . $value->img;
                    @endphp

                    @if (isset($field['temporary']))
                        <a target="_blank"
                            href="{{ asset(\Storage::disk('public')->temporaryUrl($filePath, now()->addMinutes($field['temporary']))) }}">
                            {{ $value->img }}
                        </a>
                    @else
                        <a target="_blank" href="{{ asset(\Storage::disk('public')->url($filePath)) }}">
                            {{ $value->img }}
                        </a>
                    @endif

                    <a href="#" class="btn btn-light btn-sm float-right file-clear-button" title="Clear file"
